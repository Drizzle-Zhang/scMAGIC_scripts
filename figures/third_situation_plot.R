...
for (method in vec.methods) {
    df_sub <- df_3situ[df_3situ$method == method,]
...
df_plot <- rbind(df_3situ, df_mean)
...
# Accuracy
########################################
sort.datasets <- c('Baron -> Muraro, Pancreas', 'Baron -> Segerstolpe, Pancreas', 
...
df_plot.acc$method <- factor(df_plot.acc$method, levels = (sort.methods), 
                             labels = (label.methods))
df_plot.acc$dataset <- factor(df_plot.acc$dataset, levels = (sort.datasets))

plot.heatmap <- 
    ggplot(data = df_plot.acc, aes(x = dataset, y = method)) + 
    ...
    theme(panel.background = element_rect(color = 'transparent',
                                          fill = 'transparent'), 
          axis.text.x = element_text(size = 11.5, color = "black", family = 'Arial',
                                     angle = 315, vjust = 1, hjust = 0),
          axis.text.y = element_text(
              size = 11.5, color = "black", family = 'Arial', vjust = 0.5, hjust = 0.5),
...
ggsave(filename = 'heatmap_CS.png', 
       ...
       units = 'cm', height = 14, width = 14)

# boxplot
df.box <- df_plot.acc
df.box$term <- factor(df.box$term, levels = c(c('Balanced accuracy', 'Accuracy')))

plot.box <- 
    ggplot(df.box, aes(x = method, y = value, color = term)) +
    geom_boxplot(width = 0.5, outlier.size = 0.7, coef = 5) +
...
label.methods <- gsub('scmapcell', 'scmap-cell', label.methods)
label.methods <- gsub('scmapcluster', 'scmap-cluster', label.methods)
label.methods <- gsub('seurat', 'Seurat', label.methods)
label.methods <- gsub('SVM', 'SVM_rejection', label.methods)
df_plot.acc$method <- factor(df_plot.acc$method, levels = (sort.methods), 
                             labels = (label.methods))
df_plot.acc$dataset <- factor(df_plot.acc$dataset, levels = (sort.datasets))
...
vec.methods <- c('scMAGIC', 'scmapcluster', 'scmapcell', 'SingleR', 'scClassify', 'scPred',
                 'sciBet', 'singleCellNet', 'CHETAH', 'CALLR', 'scSemiCluster', 'scID', 
                 'CaSTLe', 'SVM', 'seurat')
vec.datasets <- unique(df_3situ$dataset)
df_cluster_acc <- data.frame(stringsAsFactors = F)
for (method in vec.methods) {
    for (dataset in vec.datasets) {
        num_correct <- df_3situ[df_3situ$method == method & df_3situ$dataset == dataset & 
                                    df_3situ$term == 'Number of correctly labeled clusters', 'value']
        num_total <- df_3situ[df_3situ$method == method & df_3situ$dataset == dataset & 
                                  df_3situ$term == 'Total number of clusters', 'value']
        type <- df_3situ[df_3situ$method == method & df_3situ$dataset == dataset & 
                             df_3situ$term == 'Total number of clusters', 'type']
...
ggsave(filename = 'heatmap_CS_cluster.png', 
       path = path.fig, plot = plot.heatmap,
       units = 'cm', height = 14, width = 10)

# boxplot
df.box.acc <- df_plot.acc[df_plot.acc$Color_Type == '0',]

plot.box.acc <- 
    ggplot(df.box.acc, aes(x = method, y = value)) +
    geom_boxplot(width = 0.7, outlier.size = 1, color = "#1F78B4") +
    scale_y_continuous(breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1)) +
    labs(title = "", y = 'Cluster accuracy', x = '') + 
    coord_flip() +
    theme_bw() + 
    theme(panel.background = element_rect(color = 'black', size = 1,
                                          fill = 'transparent'),
          ...
          legend.text = element_text(size = 9, family = 'Arial'),
          legend.key = element_blank())
ggsave(filename = 'boxplot_CS_cluster.png', 
