...
# evaluation
simple.evaluation <- function(true.tag, scMAGIC.tag, df.cell.names) {
    # uniform tags
    for (j in 1:dim(df.cell.names)[1]) {
        scMAGIC.tag[scMAGIC.tag == df.cell.names[j, 'ref.name']] <- 
            df.cell.names[j, 'sc.name']
    }
    scMAGIC.tag[!(scMAGIC.tag %in% df.cell.names$sc.name)] <- 'Unassigned'
...
myces <- PredCellTypes(as.matrix(exp_sc_mat), species = 'mouse', model = 'generic_38celltypes', 
                       results.dir = '/mdshare/node9/zy/scMAGIC/methods/rSuperCT_models')
table(myces)
...
file_marker <- paste0('/mdshare/node9/zy/scMAGIC/methods/garnett/mouse_brain_', 
