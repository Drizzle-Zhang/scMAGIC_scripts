...
# evaluation
simple.evaluation <- function(true.tag, scMAGIC.tag, df.ref.names, df.sc.names) {
    # uniform tags
    for (j in 1:dim(df.ref.names)[1]) {
        scMAGIC.tag[scMAGIC.tag == df.ref.names[j, 'ref.name']] <- df.ref.names[j, 'name']
    }
    scMAGIC.tag[!(scMAGIC.tag %in% df.ref.names$name)] <- 'Unassigned'
    for (j in 1:dim(df.sc.names)[1]) {
        true.tag[true.tag == df.sc.names[j, 'sc.name']] <- df.sc.names[j, 'name']
    }
    
    percent.unassigned <- sum(scMAGIC.tag == 'Unassigned')/sum(true.tag == 'Unassigned')
...
source('/local/zy/my_git/scMAGIC/main/scMAGIC.v20.R')

############# regard sc-counts data as reference
# file.mtx <- '/mdshare/node9/zy/scMAGIC/sc_data/MouseAtlas_SingleCell_Han2018/MCA_by_tissue/Brain/Count_all_batch.txt'
# df.mtx <- read.delim(file.mtx, stringsAsFactors = F, row.names = 1)
# file.cellid <- '/mdshare/node9/zy/scMAGIC/sc_data/MouseAtlas_SingleCell_Han2018/MCA_by_tissue/Brain/CellAssignments_all_batch.txt'
...
# OUT <- list()
# OUT$mat_exp <- ref.mtx
# OUT$label <- ref.labels
ref.dataset <- 'MCA'
path.output <- '/mdshare/node9/zy/scMAGIC/Benchmark/mouse_brain/'
# saveRDS(OUT, file = paste0(path.output, ref.dataset, '.Rdata'))
OUT <- readRDS(paste0(path.output, ref.dataset, '.Rdata'))
exp_MCA <- OUT$mat_exp
label_MCA <- OUT$label
ref.mtx <- exp_MCA
ref.labels <- label_MCA

############### import unlabeled data
############### HochgernerA
file.in <- '/mdshare/node9/zy/scMAGIC/sc_data/Hochgerner/GSE95315_10X_expression_data_v2.tab'
path.output <- '/mdshare/node9/zy/scMAGIC/Benchmark/mouse_brain/'
dataset <- 'HochgernerA'
