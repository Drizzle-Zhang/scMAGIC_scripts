...
setwd('/local/zy/my_git/scMAGIC/Benchmark/cross_validation/train4_test1/Campbell')
source('./Cross_Validation.R')
source('./method_functions.R')
source('./evaluate.R')

path.input <- '/mdshare/node9/zy/scMAGIC/sc_data/'
path.output <- '/mdshare/node9/zy/scMAGIC/cross_validation/train4_test1/Campbell/'

# setwd('/home/drizzle_zhang/my_git/scMAGIC/Benchmark/cross_validation/train4_test1')
# source('./Cross_Validation.R')
# source('./method_functions.R')
# source('./evaluate.R')
# 
# path.input <- '/home/drizzle_zhang/scMAGIC/'
# path.output <- '/home/drizzle_zhang/scMAGIC/cross_validation/train4_test1/'
...
# scMAGIC
# run_scMAGIC(DataPath,LabelsPath,CV_RDataPath,OutputDir)
TrueLabelsPath <- paste0(OutputDir, 'scMAGIC_True_Labels.csv')
# PredLabelsPath <- paste0(OutputDir, 'scMAGIC_Pred_Labels_cell.csv')
PredLabelsPath <- paste0(OutputDir, 'scMAGIC_Pred_Labels.csv')
res.scMAGIC <- evaluate(TrueLabelsPath, PredLabelsPath)
df.sub <- data.frame(term = names(res.scMAGIC$F1), 
                     method = rep('scMAGIC', length(res.scMAGIC$F1)),
                     value = res.scMAGIC$F1, stringsAsFactors = F)
df.sub <- rbind(df.sub, 
                data.frame(term = 'macro F1', method = 'scMAGIC',
                           value = res.scMAGIC$Mean_F1, stringsAsFactors = F))
df.sub <- rbind(df.sub, 
                data.frame(term = 'Accuracy', method = 'scMAGIC',
                           value = res.scMAGIC$Acc, stringsAsFactors = F))
df.sub <- rbind(df.sub, 
                data.frame(term = 'Weighted macro F1', method = 'scMAGIC',
                           value = res.scMAGIC$WMean_F1, stringsAsFactors = F))
df.sub <- rbind(df.sub, 
                data.frame(term = 'Balanced accuracy', method = 'scMAGIC',
                           value = res.scMAGIC$balanced_acc, stringsAsFactors = F))
